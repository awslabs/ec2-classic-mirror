...
function _cleanupFailedGroupPromise(pair) {
    if (pair.createdVpcSecurityGroupId) {
        ...
        if (pair.classicSecurityGroup.GroupName == 'default') {
            console.log('CreateSecurityGroups: Clean up tags for default Security Group ' + pair.createdVpcSecurityGroup + '; not deleting because default');
        } else {
            console.log('CreateSecurityGroups: Clean up failed creation of ' + pair.createdVpcSecurityGroupId);
            deleteSecurityGroupPromise = ec2DeleteSecurityGroup({GroupId: pair.createdVpcSecurityGroupId});
        }

        return deleteSecurityGroupPromise.then(function() {
